...
1. **Create the scenario folder** (if it does not exist):
	...
	```powershell
	New-Item -ItemType Directory -Path "app/packing-lists/{exporter}/test-scenarios/basic-tests" -Force
	```

2. **Copy the happy path sample file** to each scenario filename in `app/packing-lists/{exporter}/test-scenarios/basic-tests/` using PowerShell or CLI. Do not create blank files from scratch. Example:

	```powershell
	Copy-Item "app/packing-lists/{exporter}/HappyPath.xlsx" "app/packing-lists/{exporter}/test-scenarios/basic-tests/Missing_OptionalHeader_All_Pass.xlsx"
...
5. **After mutation,** verify that the file is no longer identical to the template (except for Happypath.xlsx).
6. **Track mutation progress** using PowerShell commands to ensure all files except Happypath.xlsx have been modified.
...
## Documentation: Basic Tests Scenario Types

- **Happypath.xlsx**: Exact copy of the input file for baseline validation.
- **Missing_OptionalHeader_All_Pass.xlsx**: Remove or rename all optional column headers so they do not match the original regex patterns.
- **Missing_OptionalData_All_Pass.xlsx**: Clear all data in optional columns while preserving headers.
- **Incorrect_OptionalData_All_Pass.xlsx**: Insert invalid/incorrect data in all optional columns.
- **Incorrect_OptionalHeader_All_Pass.xlsx**: Modify optional column headers to incorrect but non-matching values.
- **OnlyMandatoryDataIsFilled_Pass.xlsx**: Clear all optional data while keeping mandatory data intact.
- **DescriptionHasDoubleQuotesShould_Pass.xlsx**: Add double quotes to description field data to test special character handling.
- **MandatoryHeaders_CaseInSensitive_Pass.xlsx**: Change the case of mandatory headers to test case/formatting variations.
- **Incorrect_Mandatatypes_Excl_netandNopkgs_ProductCode_Pass.xlsx**: Insert non-standard data types in non-critical mandatory fields excluding net weight and number of packages.
- **Incorrect_MandatoryHeader_CommodityCode_Unparse.xlsx**: Remove the header name for the commodity_code column.
- **Incorrect_MandatoryHeader_All_Unparse.xlsx**: Remove ALL mandatory header names.
- **Incorrect_MandatoryHeader_Desc_Unparse.xlsx**: Remove the header name for the description column.
- **Incorrect_MandatoryHeader_TotNetweight_Unparse.xlsx**: Remove the header name for the total_net_weight_kg column.
- **Incorrect_MandatoryHeader_NoofPakgs_Unparse.xlsx**: Remove the header name for the number_of_packages column.
- **Incorrect_MandatoryHeader_TreatmentType_Unparse.xlsx**: Remove the header name for the type_of_treatment column.
- **Incorrect_MandatoryHeader_CommodityCode_Unparse.xlsx**: Alternative test for commodity code header removal.
- **Incorrect_MandatoryHeader_TotNetweightKGS_Fail.xlsx**: Modify the net weight header to use different unit terminology that doesn't match the regex.
- **Empty_MultipleRowsColumns_Pass.xlsx**: Include empty rows in the data section while maintaining valid structure.
- **Missing_MandatoryHeader_All_Unparse.xlsx**: Remove ALL mandatory header names completely.
- **Missing_MandatoryHeader_Description_unparse.xlsx**: Remove only the description header.
- **Missing_MandatoryHeader_CommodityCode_Unparse.xlsx**: Remove only the commodity code header.
- **Missing_MandatoryHeader_NoofPacakges_Unparse.xlsx**: Remove only the number of packages header.
- **Missing_MandatoryHeader_TotNetWeight_Unparse.xlsx**: Remove only the total net weight header.
- **Incorrect_MandatoryData_MultipleRowsWithMultipleLocations_All_Fail.xlsx**: Insert invalid data types in mandatory fields across multiple rows and locations.
- **Missing_MandatoryData_MultipleRowsWithMultipleLocations_All_Fail.xlsx**: Clear mandatory data across multiple rows and locations.
- **Missing_MandatoryData_CommodityCode_Fail.xlsx**: Clear commodity code data in multiple rows.
- **Missing_MandatoryData_CommodityCode_Nature_Fail.xlsx**: Clear both commodity code and nature of products data.
- **Missing_MandatoryData_Desc_Fail.xlsx**: Clear description data in multiple rows.
- **Missing_MandatoryData_Noofpkgs_Fail.xlsx**: Clear number of packages data in multiple rows.
- **Missing_MandatoryData_Totnetweight_Fail.xlsx**: Clear total net weight data in multiple rows.
- **AllMandatoryDataIsMissing_Fail.xlsx**: Clear ALL mandatory data while keeping headers.
- **NoData_ExceptSingleRMS_Fail.xlsx**: Remove all data rows except establishment number information.
- **InvalidCommodityCode_MultipleRows_Fail.xlsx**: Insert invalid commodity code formats across multiple rows.

## Output
- Place all generated files in `app/packing-lists/{exporter}/test-scenarios/basic-tests/`.
- Ensure all files except Happypath.xlsx have appropriate mutations applied.
