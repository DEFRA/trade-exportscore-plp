...
describe("matchesTJMorrisModel1", () => {
  ...
  test("matches valid TJMorris Model 1 file, calls parser and returns all_required_fields_present as false for multiple rms", async () => {
  ...
  test("matches valid TJMorris Model 1 file, calls parser and returns all_required_fields_present as false for missing kg unit", async () => {
    const result = await parserService.findParser(
      model.missingKgunit,
      filename,
    );
