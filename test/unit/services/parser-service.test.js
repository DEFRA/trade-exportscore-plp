...
describe("matchesFowlerWelch2", () => {
  test.only("returns generic error for empty json", () => {
    const packingListJson = {};
    const filename = "packinglist.xlsx";
    const result = parserService.matchesFowlerWelch2(packingListJson, filename);
    expect(result).toBe(MatcherResult.GENERIC_ERROR);
  });
  test.only("returns wrong extension for incorrect file extension", () => {
    const filename = "packinglist.xls";
    const packingListJson = {};
    const result = parserService.matchesFowlerWelch2(packingListJson, filename);
    expect(result).toBe(MatcherResult.WRONG_EXTENSIONS);
  });
  test.only("returns wrong establishment number for missing establishment number", () => {
    const packingListJson = {
      "Cust Ord - Vitacress": [
        {},
        {},
  ...
  test.only("returns wrong header for incorrect header values", () => {
    const filename = "packinglist.xlsx";
    const packingListJson = {
      "Cust Ord - Vitacress": [
        {},
  ...
  test.only("returns correct for correct headers", () => {
    const filename = "packinglist.xlsx";
    const packingListJson = {
      "Cust Ord - Vitacress": [
        {},
  ...
  describe("parseFowlerWelch2", () => {
    test.only("parses json", () => {
      ...
      const result = parserService.parseFowlerWelch2(packingListJson);
      expect(result.registration_approval_number).toBe(packingListJson[45].M);
      expect(result.items).toHaveLength(2);
      expect(result.items[0].description).toBe(packingListJson[45].F);
      expect(result.items[1].description).toBe(packingListJson[46].F);
