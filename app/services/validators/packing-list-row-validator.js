...
function validatePackingListByIndex(packingList) {
  ...
  const itemFailures = packingList.items
    .map((val, index) => {
      let result = {};
...
function generateFailuresByIndex(validationResult) {
  ...
  if (!validationResult.hasAllFields) {
    ...
    if (validationResult.isEmpty)
      failureReasons = failureReasons.concat("The packing list had no data.  ");
    if (!validationResult.hasRemos)
      failureReasons = failureReasons.concat("Remos number missing.  ");
    if (validationResult.itemFailures.length > 0)
      failureReasons = failureReasons.concat(
        generateitemsFailureReasons(validationResult.itemFailures),
      );
  ...
  if (items.length > maxFailuresToShow) {
    ...
    return (failureReasons = failureReasons.concat(
      items
        ...
        .reduce(
          (prev, current) => prev.concat(generateitemFailureReasons(current)),
          "",
        ),
    ));
  } else {
...
function generateitemFailureReasons(item) {
  ...
  if (item.wrongTypeForNetWeight)
    failureDetails += "Wrong Type for Net Weight.  ";
