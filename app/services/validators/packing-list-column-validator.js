...
function generateFailuresByIndexAndTypes(validationResult, packingList) {
  if (validationResult.hasAllFields && validationResult.hasSingleRms) {
    ...
    if (packingList.unitInHeader) {
      addNetWeightUnitBlanketFailureReason(validationResult, failuresAndChecks);
    } else {
      addNetWeightUnitCheck(validationResult, failuresAndChecks);
    }
    ...
    if (packingList.blanketNirms) {
      ...
      addNirmsCheck(validationResult, failuresAndChecks);
    }
...
/**
 * Append a blanket net-weight-unit failure reason when unit is in header.
 * @param {Object} validationResult - Validation summary object.
 * @param {Object} reasonsAndChecks - Accumulator containing `failureReasons` and `checks`.
 */
function addNetWeightUnitBlanketFailureReason(
  validationResult,
  reasonsAndChecks,
) {
...
/**
 * Add a check entry for rows needing net weight unit attention when unit is not in header.
 * @param {Object} validationResult - Validation summary object.
 * @param {Object} reasonsAndChecks - Accumulator containing `failureReasons` and `checks`.
 */
function addNetWeightUnitCheck(validationResult, reasonsAndChecks) {
...
/**
 * Append a blanket NIRMS failure reason when blanket statement is present.
 * @param {Object} validationResult - Validation summary object.
 * @param {Object} reasonsAndChecks - Accumulator containing `failureReasons` and `checks`.
 */
...
/**
 * Add a check entry for per-row missing NIRMS when no blanket statement exists.
 * @param {Object} validationResult - Validation summary object.
 * @param {Object} reasonsAndChecks - Accumulator containing `failureReasons` and `checks`.
 */
function addNirmsCheck(validationResult, reasonsAndChecks) {
  reasonsAndChecks.checks.push({
