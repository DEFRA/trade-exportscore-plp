...
- `data-prohibited-items.json` — Current consolidated list of prohibited items and categories used by business validation checks.
- `data-prohibited-items/` — Versioned prohibited items files. Each file is a snapshot used for historic or versioned validation.

Usage notes

- Consumers should treat these files as read-only reference data.
- When updating prohibited items, add a new versioned file under `data-prohibited-items/` and update `data-prohibited-items.json` to reference the new version if needed.
